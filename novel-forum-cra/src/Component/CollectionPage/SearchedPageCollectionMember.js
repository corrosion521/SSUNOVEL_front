...
const SearchedPageCollectionMember = () => {
    //------페이지네이션-----------------------------------------
    ...
    useEffect(() => {
        ...
        fetch(`/box/search?item=${searchTerm}&page=${currentPage}`, {
            ...
            .then((result) => {
                ...
                // 전체 페이지 수 계산
                setTotalPages(Math.ceil(result.result.boxCntByMember / itemsPerPage))
            });
...
export default SearchedPageCollectionMember;